...
export default function PageSertifikatKader() {
  ...
  useEffect(() => {
    ...
    const unsubMaster = onSnapshot(qMaster, (snapshot) => {
      if (!snapshot.empty) {
        ...
        setSettings(prev => ({
          ...prev, templateUrl: masterData.templateUrl || '', orientasi: masterData.orientasi || 'portrait',
          posisi: loadedPosisi, namaKetuaCabang: masterData.namaKetuaCabang || '',
          stempelCabangUrl: masterData.stempelCabangUrl || '', scanTtdCabangUrl: masterData.scanTtdCabangUrl || '',
          namaKetuaKomisariat: masterData.namaKetuaKomisariat || '', stempelKomisariatUrl: masterData.stempelKomisariatUrl || '',
    ...
    if (profilKader.id_rayon) {
      ...
      const unsubRayonConf = onSnapshot(doc(db, "pengaturan_sertifikat", rayonDocId), (docSnap) => {
        if (docSnap.exists()) {
          ...
          setSettings(prev => ({
            ...prev,
            namaKetuaRayon: rayonData.namaKetuaRayon || '', stempelRayonUrl: rayonData.stempelUrl || '',
            scanTtdRayonUrl: rayonData.scanTtdRayonUrl || '', masaKhidmat: rayonData.masaKhidmat || '',
            tempatDitetapkan: rayonData.tempatDitetapkan || '', tanggalMasehi: rayonData.tanggalMasehi || '',
            tanggalHijriyah: rayonData.tanggalHijriyah || '', tanggalPelaksanaan: rayonData.tanggalPelaksanaan || '',
            tempatPelaksanaan: rayonData.tempatPelaksanaan || ''
          }));
        } else {
          setSettings(prev => ({
            ...prev,
            namaKetuaRayon: '', stempelRayonUrl: '', scanTtdRayonUrl: '', masaKhidmat: '', tempatDitetapkan: '',
            tanggalMasehi: '', tanggalHijriyah: '', tanggalPelaksanaan: '', tempatPelaksanaan: ''
          }));
        }
      });
  ...
  const canvasWidth = settings.orientasi === 'portrait' ? 794 : 1123;
  const canvasHeight = settings.orientasi === 'portrait' ? 1123 : 794;
  ...
  return (
    <>
      <style>{`
        ...
        /* Container khusus pratinjau responsif absolut */
        ...
        @media (max-width: 767px) {
           ...
           .web-ui-container { overflow: visible !important; }
        }
        ...
        @media print {
          ...
          .isian-data { position: absolute; z-index: 10; font-family: "Arial", Helvetica, sans-serif; color: black !important; line-height: 1.3; margin: 0; }
      ...
      <div className="web-ui-container page-wrapper">
        ...
        <div className="preview-card">
          ...
          {/* Scroll Container mencegah responsif hancur di HP */}
          <div className="preview-scroll-container">
            <div style={{ 
              position: 'relative', width: `${canvasWidth}px`, minWidth: `${canvasWidth}px`, height: `${canvasHeight}px`,
              margin: '0 auto', border: '1px solid var(--border-color)', borderRadius: '6px', 
              overflow: 'hidden', backgroundColor: '#fdfdfd', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)'
            }}>
              ...
              {/* Render Teks Dinamis dengan Font Arial */}
              {dataSertifikatKader && Object.keys(settings.posisi).map(key => {
                ...
                return (
                  <div key={key} style={{ 
                    ...
                    fontFamily: '"Arial", Helvetica, sans-serif', fontSize: `${(p.fontSize / 650) * canvasWidth}px`, 
