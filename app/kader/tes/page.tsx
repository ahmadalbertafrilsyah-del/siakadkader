    ...
    <>
      <style>{`
        @media (max-width: 767px) {
           ...
           /* Memberikan padding yang lega di HP */
           .mobile-card-padding {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div className="mobile-card-padding" style={{ background: 'white', padding: '25px', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', minHeight: '80vh', border: '1px solid #eaeaea' }}>
          ...
          {selectedTes ? (
            <div>
               <div style={{ borderBottom: '2px solid #eee', paddingBottom: '15px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '15px' }}>
                 ...
                 }} style={{ backgroundColor: '#f8f9fa', color: '#333', border: '1px solid #ccc', padding: '8px 15px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.85rem' }}>
                   ❌ Batal Kerjakan
                 </button>
               </div>
               ...
               <form onSubmit={handleSubmitJawaban} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                 {(selectedTes.daftar_soal || []).map((soal: string, idx: number) => (
                   <div key={idx} style={{ backgroundColor: '#fdfdfd', padding: '20px', border: '1px solid #eee', borderRadius: '8px', borderLeft: '4px solid #0000af', boxShadow: '0 2px 5px rgba(0,0,0,0.02)' }}>
                     ...
                     }} style={{ width: '100%', padding: '15px', border: '1px solid #ccc', borderRadius: '6px', fontSize: '0.9rem', resize: 'vertical', outline: 'none', boxSizing: 'border-box', backgroundColor: '#fff', fontFamily: 'inherit', transition: 'border-color 0.3s' }} />
                   </div>
                 ))}
                 <button disabled={isSubmitting} type="submit" style={{ backgroundColor: '#0000af', color: 'white', border: 'none', padding: '15px', borderRadius: '8px', fontWeight: 'bold', cursor: isSubmitting ? 'not-allowed' : 'pointer', fontSize: '1rem', marginTop: '10px', transition: '0.2s', boxShadow: '0 4px 10px rgba(0,0,175,0.2)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
                {listTesTersedia.length === 0 ? (
                  ...
                  listTesTersedia.map(tes => {
                    ...
                    return (
                      <div key={tes.id} style={{ backgroundColor: '#fff', border: '1px solid #eee', padding: '20px', borderRadius: '10px', boxShadow: '0 4px 12px rgba(0,0,0,0.03)', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        <div>
                        ...
                        {sudahDikerjakan ? (
                          <div style={{ backgroundColor: '#eaf4fc', color: '#27ae60', padding: '12px', borderRadius: '6px', textAlign: 'center', fontWeight: 'bold', border: '1px solid #2ecc71', fontSize: '0.85rem' }}>
                            ✅ Anda sudah menyelesaikan tes ini.
                          </div>
                        ) : (
                          <button onClick={() => handleMulaiTes(tes)} style={{ backgroundColor: '#0000af', color: 'white', border: 'none', padding: '12px', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.85rem', transition: '0.2s', boxShadow: '0 2px 8px rgba(0,0,175,0.2)' }}>
