...
export default function PagePengaturanSertifikatKomisariat() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, "master_template_sertifikat", docId), (docSnap) => {
      if (docSnap.exists()) {
        ...
        setTemplateUrl(''); setOrientasi('portrait'); setPosisi(defaultPosisi);
  ...
  const handleSimpanPengaturan = async () => {
    ...
    try {
      let finalTemplate = templateUrl; let finalStempelCab = stempelCabangUrl; let finalScanTtdCab = scanTtdCabangUrl;
      let finalStempelKom = stempelKomisariatUrl; let finalScanTtdKom = scanTtdKomisariatUrl;
      ...
      await setDoc(doc(db, "master_template_sertifikat", docId), {
        id_komisariat: adminKomisariatId, jenjang: formJenjang, angkatan: formAngkatan, orientasi, posisi,
        templateUrl: finalTemplate, namaKetuaCabang, stempelCabangUrl: finalStempelCab, scanTtdCabangUrl: finalScanTtdCab,
  ...
  const handleSalinTemplateMaster = async () => {
    ...
    if (sourceSnap.exists()) {
      ...
      if (data.posisi) setPosisi(data.posisi); if (data.orientasi) setOrientasi(data.orientasi);
      if (data.namaKetuaCabang) setNamaKetuaCabang(data.namaKetuaCabang); if (data.namaKetuaKomisariat) setNamaKetuaKomisariat(data.namaKetuaKomisariat);
      if (data.stempelCabangUrl) setStempelCabangUrl(data.stempelCabangUrl); if (data.scanTtdCabangUrl) setScanTtdCabangUrl(data.scanTtdCabangUrl);
      if (data.stempelKomisariatUrl) setStempelKomisariatUrl(data.stempelKomisariatUrl); if (data.scanTtdKomisariatUrl) setScanTtdKomisariatUrl(data.scanTtdKomisariatUrl);
  ...
  const handleMouseDown = (e: React.MouseEvent, key: string) => { e.stopPropagation(); setDraggingKey(key); };
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    newLeft = Math.max(0, Math.min(95, newLeft)); newTop = Math.max(0, Math.min(95, newTop));
  ...
  return (
    <>
      <style>{`
        ...
        /* Container khusus pratinjau absolut */
      ...
      <div className="pengaturan-komisariat-wrapper">
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px' }}>
              ...
              {Object.keys(posisi).map((key) => {
                ...
                return (
                  <div key={key} className="kordinat-row">
                    ...
                    <div style={{ display: 'flex', gap: '2px', alignItems: 'center' }}><span style={{ color: '#777', fontWeight: 'bold' }}>X:</span><input type="number" step="0.1" value={p.left ?? ''} onChange={e => updatePosisi(key, 'left', Number(e.target.value))} style={{ width: '100%', padding: '4px', border: '1px solid #ccc', borderRadius: '4px' }} />%</div>
                    <div style={{ display: 'flex', gap: '2px', alignItems: 'center' }}><span style={{ color: '#777', fontWeight: 'bold' }}>Y:</span><input type="number" step="0.1" value={p.top ?? ''} onChange={e => updatePosisi(key, 'top', Number(e.target.value))} style={{ width: '100%', padding: '4px', border: '1px solid #ccc', borderRadius: '4px' }} />%</div>
                    <div style={{ display: 'flex', gap: '2px', alignItems: 'center' }}><span style={{ color: '#777', fontWeight: 'bold' }}>W:</span><input type="number" step="1" value={p.width ?? ''} onChange={e => updatePosisi(key, 'width', Number(e.target.value))} style={{ width: '100%', padding: '4px', border: '1px solid #ccc', borderRadius: '4px' }} />%</div>
                    <div style={{ display: 'flex', gap: '2px', alignItems: 'center' }}><span style={{ color: '#777', fontWeight: 'bold' }}>Sz:</span><input type="number" value={p.fontSize ?? ''} onChange={e => updatePosisi(key, 'fontSize', Number(e.target.value))} style={{ width: '100%', padding: '4px', border: '1px solid #ccc', borderRadius: '4px' }} /></div>
                    <div>
          ...
          {/* LIVE PREVIEW KOMISARIAT WADAH ABSOLUT */}
            ...
            <div className="preview-scroll-container">
              <div 
                ...
                style={{ 
                  ...
                  margin: '0 auto', backgroundColor: 'white', border: '2px solid #ccc', boxShadow: '0 5px 15px rgba(0,0,0,0.1)', 
                  overflow: 'hidden', cursor: draggingKey ? 'grabbing' : 'default'
                }}
              >
                ...
                {/* Teks Dinamis Master dengan Arial Font */}
                {Object.keys(posisi).map(key => {
                  ...
                  return (
                    <div key={key} onMouseDown={e => handleMouseDown(e, key)} style={{ 
                      ...
                      textAlign: p.align || (isCenter ? 'center' : 'left'), transform: isCenter ? 'translate(-50%, 0)' : 'none', cursor: 'grab',
                      ...
                      color: '#000', lineHeight: '1.3', border: '1px dashed rgba(255,0,0,0.5)', background: 'rgba(255,255,255,0.6)', padding: '2px 4px', margin: 0
                    }}>
