...
export default function DashboardAdminRayon() {
  ...
  const barisRaportRender = materiAktif.map((materi, index) => {
    ...
    return (
      <tr key={materi.kode} className="row-tabel-cetak" style={{ borderBottom: '1px solid #ccc', backgroundColor: index % 2 === 0 ? '#fff' : '#fafafa' }}>
        <td className="col-cetak" style={{ textAlign: 'center', padding: '10px 15px' }}>{index + 1}</td>
        <td className="col-cetak" style={{ padding: '10px 15px', color: '#333' }}>{materi.kode}</td>
        <td className="col-cetak" style={{ padding: '10px 15px', color: '#333' }}>{materi.nama}</td>
        <td className="col-cetak" style={{ padding: '10px 15px', textAlign: 'center', color: '#333' }}>{materi.bobot}</td>
        <td className="no-print" style={{ padding: '10px 15px', textAlign: 'center' }}>
        ...
        <td className="print-only-header col-cetak" style={{ textAlign: 'center', padding: '10px 15px', color: '#333' }}>
           {nilaiHuruf === "-" ? "" : nilaiHuruf}
        </td>
        <td className="col-cetak" style={{ padding: '10px 15px', textAlign: 'center', color: '#333' }}>{nilaiHuruf === '-' ? 0 : sksKaliNilai}</td>
      </tr>
    );
  });
  ...
  // ==========================================
  // FUNGSI VERIFIKASI & UPLOAD SURAT BALASAN MANUAL
  // ==========================================
  ...
  const handleBuatAkunKader = async (e: React.FormEvent) => {
    ...
    try {
      const emailBaru = `${formKader.nim}@sikad.com`.toLowerCase();
  ...
  const handleBuatAkunPendamping = async (e: React.FormEvent) => {
    ...
    const secondaryAuth = getSecondaryAuth();
    try {
      const emailBaru = `${formPendamping.username}@sikad.com`.toLowerCase();
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          const emailBaru = `${nim}@sikad.com`.toLowerCase();
          try {
    ...
    <div style={{ display: 'flex', backgroundColor: '#f4f6f9', minHeight: '100vh', fontFamily: 'Arial, sans-serif' }}>
      ...
      <style>{`
        ...
        @media print {
          ...
          .tabel-cetak th, .tabel-cetak td.col-cetak { 
          ...
          .tabel-cetak th { font-weight: bold !important; text-align: center !important; }
          ...
          .print-kop-surat { display: block !important; margin-bottom: 15px; width: 100%; } 
          .print-kop-surat img { width: 100% !important; max-height: 250px; object-fit: contain; }
          
          .print-footer { display: flex !important; width: 100%; justify-content: center; margin-top: 30px; page-break-inside: avoid; } 
          .print-footer img { width: 100% !important; object-fit: contain; }
      ...
      <main style={{ 
        ...
        <div style={{ padding: '20px', flex: 1 }}>
          ...
          {activeMenu === 'beranda' && (
            <div>
              <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: '8px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', marginBottom: '20px' }}>
                <h2 style={{color: '#1e824c', marginTop: 0}}>Selamat Datang di Pusat Kendali {namaRayonAsli}!</h2>
                <p style={{color: '#555', lineHeight: '1.6', margin: 0}}>Kelola data kaderisasi, master tugas, surat, akun kader, dan perpustakaan secara real-time melalui panel ini.</p>
              </div>

              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px', marginBottom: '20px' }}>
                <h3 style={{ color: '#0d1b2a', margin: 0 }}>📊 Overview Kaderisasi Rayon</h3>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <select 
                    ...
                    style={{ padding: '8px 15px', borderRadius: '4px', border: '1px solid #1e824c', fontWeight: 'bold', color: '#1e824c', outline: 'none', cursor: 'pointer' }}
                  >
              ...
              <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', marginBottom: '30px' }}>
                <div style={{ flex: '1 1 200px', backgroundColor: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', borderBottom: '4px solid #3498db' }}>
                  <div style={{ color: '#7f8c8d', fontSize: '0.9rem', fontWeight: 'bold' }}>Total Kader Terdata</div>
                  <div style={{ fontSize: '2.5rem', fontWeight: 'bold', color: '#2c3e50', marginTop: '10px' }}>{dataKaderDifilterTahun.length}</div>
                </div>
                <div style={{ flex: '1 1 200px', backgroundColor: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', borderBottom: '4px solid #2ecc71' }}>
                  <div style={{ color: '#7f8c8d', fontSize: '0.9rem', fontWeight: 'bold' }}>Total Pendamping Aktif</div>
                  <div style={{ fontSize: '2.5rem', fontWeight: 'bold', color: '#2c3e50', marginTop: '10px' }}>{dataPendamping.length}</div>
                </div>
                <div style={{ flex: '1 1 200px', backgroundColor: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', borderBottom: '4px solid #f1c40f' }}>
                  <div style={{ color: '#7f8c8d', fontSize: '0.9rem', fontWeight: 'bold' }}>Surat Menunggu</div>
                  <div style={{ fontSize: '2.5rem', fontWeight: 'bold', color: '#2c3e50', marginTop: '10px' }}>{suratMasuk.filter(s => s.status === 'Menunggu Verifikasi').length}</div>
                </div>
                <div style={{ flex: '1 1 200px', backgroundColor: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', borderBottom: '4px solid #e74c3c' }}>
                  <div style={{ color: '#7f8c8d', fontSize: '0.9rem', fontWeight: 'bold' }}>Tugas Rayon Aktif</div>
                  <div style={{ fontSize: '2.5rem', fontWeight: 'bold', color: '#2c3e50', marginTop: '10px' }}>{listMasterTugas.length}</div>
                </div>
              </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.9rem', minWidth: '400px' }}>
                  <thead>
                    <tr style={{ backgroundColor: '#f8f9fa', color: '#555' }}>
                      <th style={{ padding: '12px', borderBottom: '2px solid #ddd' }}>Jenjang Kaderisasi</th>
                      <th style={{ padding: '12px', borderBottom: '2px solid #ddd', textAlign: 'center' }}>Jumlah Kader</th>
                    </tr>
                  </thead>
                  <tbody>
                    {['MAPABA', 'PKD', 'SIG', 'SKP'].map((jenjang) => {
                      let count = 0;
                      if (jenjang === 'MAPABA') {
                        count = dataKaderDifilterTahun.filter(k => ['MAPABA', 'PKD', 'SIG', 'SKP'].includes(k.jenjang)).length;
                      } else if (jenjang === 'PKD') {
                        count = dataKaderDifilterTahun.filter(k => ['PKD', 'SKP'].includes(k.jenjang)).length;
                      } else if (jenjang === 'SIG') {
                        count = dataKaderDifilterTahun.filter(k => ['SIG', 'SKP'].includes(k.jenjang)).length;
                      } else if (jenjang === 'SKP') {
                        count = dataKaderDifilterTahun.filter(k => k.jenjang === 'SKP').length;
                      }

                      return (
                        <tr key={jenjang} style={{ borderBottom: '1px solid #eee' }}>
                          <td style={{ padding: '12px', fontWeight: 'bold', color: '#0d1b2a' }}>{jenjang}</td>
                          <td style={{ padding: '12px', textAlign: 'center', fontWeight: 'bold', color: '#3498db' }}>{count} Kader</td>
                        </tr>
                      )
                    })}
                  </tbody>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  ...
                  <div style={{ flex: '1 1 300px', backgroundColor: '#fdfdfd', padding: '20px', border: '1px solid #eee', borderRadius: '8px' }}>
                    ...
                    <form onSubmit={handleTambahJenisSurat} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                      <div>
                        ...
                        <input type="text" placeholder="Misal: Surat Keterangan Aktif" required value={newJenisSurat} onChange={(e) => setNewJenisSurat(e.target.value)} style={{ padding: '10px', border: '1px solid #ccc', borderRadius: '4px', width: '100%' }} />
                      </div>
                      <div>
                        ...
                        <textarea rows={3} placeholder="Misal: Sebutkan nama kegiatan, tempat, dan tanggal pelaksanaan di kolom Keperluan." value={newSyaratSurat} onChange={(e) => setNewSyaratSurat(e.target.value)} required style={{ padding: '10px', border: '1px solid #ccc', borderRadius: '4px', width: '100%', resize: 'vertical' }} />
                        <span style={{fontSize: '0.7rem', color: '#888'}}>Kader wajib membaca instruksi ini saat mengajukan surat.</span>
                      </div>
                      <button disabled={isSavingJenisSurat} type="submit" style={{ backgroundColor: '#2ecc71', color: 'white', padding: '12px', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: isSavingJenisSurat ? 'not-allowed' : 'pointer', marginTop: '5px' }}>
                      ...
                      <tbody>
                        {listJenisSurat.length === 0 ? (<tr><td colSpan={3} style={{textAlign: 'center', padding: '20px'}}>Belum ada jenis surat. Kader tidak bisa mengajukan surat.</td></tr>) : listJenisSurat.map((surat) => (
                          <tr key={surat.id} style={{ borderBottom: '1px solid #eee' }}>
                            ...
                            <td style={{ padding: '15px', textAlign: 'center' }}><button onClick={() => handleHapusJenisSurat(surat.id)} style={{ color: 'white', backgroundColor: '#e74c3c', padding: '5px 10px', borderRadius: '4px', border: 'none', cursor: 'pointer', fontWeight: 'bold' }}>Hapus</button></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <div style={{ backgroundColor: '#fff', borderRadius: '8px', border: '1px solid #ddd', minHeight: '400px', overflow: 'hidden' }}>
                <div style={{ backgroundColor: '#4a637d', padding: '15px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'white', flexWrap: 'wrap', gap: '10px' }}>
                  <span style={{ fontSize: '0.95rem', fontWeight: 'bold', letterSpacing: '1px' }}>DAFTAR PENGAJUAN SURAT KADER</span>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.8rem', backgroundColor: 'rgba(255,255,255,0.2)', padding: '5px 10px', borderRadius: '20px' }}>
                ...
                <div style={{ padding: '20px' }}>
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem', color: '#333', minWidth: '800px' }}>
                      <thead>
                        <tr style={{ borderBottom: '2px solid #ddd', backgroundColor: '#f8f9fa' }}>
                          <th style={{ padding: '12px 10px', fontWeight: 'bold' }}>NIM / Kader</th>
                          <th style={{ padding: '12px 10px', fontWeight: 'bold' }}>Jenis Pengajuan</th>
                          <th style={{ padding: '12px 10px', fontWeight: 'bold' }}>Isian Keperluan / Jawaban Syarat</th>
                          <th style={{ padding: '12px 10px', fontWeight: 'bold' }}>Status</th>
                          <th style={{ padding: '12px 10px', fontWeight: 'bold' }}>Upload Balasan Surat Manual</th>
                          <th style={{ padding: '12px 10px', fontWeight: 'bold', textAlign: 'center' }}>Aksi</th>
                        </tr>
                      </thead>
                      <tbody>
                        {suratMasuk.length === 0 ? (
                          ...
                          suratMasuk.map((surat, index) => (
                            <tr key={surat.id} style={{ borderBottom: '1px solid #eee', backgroundColor: index % 2 === 0 ? '#fafafa' : '#fff' }}>
                              <td style={{ padding: '12px 10px' }}><span style={{ fontWeight: 'bold', color: '#004a87' }}>{surat.email_kader.split('@')[0]}</span><br/><span style={{fontSize: '0.7rem', color: '#999'}}>{surat.tanggal}</span></td>
                              <td style={{ padding: '12px 10px', fontWeight: 'bold' }}>{surat.jenis}</td>
                              <td style={{ padding: '12px 10px', whiteSpace: 'pre-wrap', fontStyle: 'italic', color: '#555' }}>"{surat.keperluan}"</td>
                              <td style={{ padding: '12px 10px', fontWeight: 'bold', color: surat.status === 'Menunggu Verifikasi' ? '#f39c12' : surat.status === 'Disetujui' ? '#27ae60' : '#c0392b' }}>{surat.status}</td>
                              ...
                              <td style={{ padding: '12px 10px' }}>
                                {surat.status === 'Menunggu Verifikasi' ? (
                                  <input 
                                    ...
                                    style={{ padding: '5px', fontSize: '0.75rem', maxWidth: '180px' }}
                                  />
                              ...
                              <td style={{ padding: '12px 10px', textAlign: 'center' }}>
                                {surat.status === 'Menunggu Verifikasi' ? (
                                  <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
                                    <button disabled={isUploadingSurat} onClick={() => handleAksiSurat(surat.id, 'Disetujui')} style={{ background: '#27ae60', border: 'none', color: 'white', cursor: isUploadingSurat ? 'not-allowed' : 'pointer', fontWeight: 'bold', padding: '6px 12px', borderRadius: '4px' }}>
                                      {isUploadingSurat ? '...' : 'Setujui & Kirim'}
                                    </button>
                                    <button disabled={isUploadingSurat} onClick={() => handleAksiSurat(surat.id, 'Ditolak')} style={{ background: '#c0392b', border: 'none', color: 'white', cursor: isUploadingSurat ? 'not-allowed' : 'pointer', fontWeight: 'bold', padding: '6px 12px', borderRadius: '4px' }}>
                                      Tolak
                                    </button>
                                  </div>
                    ...
                    <p style={{fontSize: '0.8rem', color: '#888', marginTop: '15px'}}>*Untuk menyetujui surat, Anda wajib mengupload file balasan yang sudah ditandatangani secara manual (PDF/Foto) terlebih dahulu.</p>
                  </div>
                </div>
              </div>
          ...
          {activeMenu === 'manajemen-akun' && (
            <div style={{ background: 'white', padding: '20px', borderRadius: '8px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
              <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap' }}>
                <button onClick={() => setTabAkun('kader')} style={{ padding: '10px 15px', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer', backgroundColor: tabAkun === 'kader' ? '#1e824c' : '#f4f6f9', color: tabAkun === 'kader' ? 'white' : '#555', flex: '1 1 auto', textAlign: 'center' }}>🎓 Akun Kader</button>
                <button onClick={() => setTabAkun('pendamping')} style={{ padding: '10px 15px', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer', backgroundColor: tabAkun === 'pendamping' ? '#1e824c' : '#f4f6f9', color: tabAkun === 'pendamping' ? 'white' : '#555', flex: '1 1 auto', textAlign: 'center' }}>👤 Akun Pendamping</button>
              </div>
                ...
                <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
                  ...
                  <div style={{ flex: '1 1 300px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    <div style={{ backgroundColor: '#fdfdfd', padding: '20px', border: '1px solid #eee', borderRadius: '8px' }}>
                      ...
                      <form onSubmit={handleBuatAkunKader} style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '10px' }}>
                        <input type="number" placeholder="NIM Kader" value={formKader.nim} onChange={e => setFormKader({...formKader, nim: e.target.value})} required style={{ padding: '10px', border: '1px solid #ccc', borderRadius: '4px' }} />
                        <input type="text" placeholder="Nama Lengkap Kader" value={formKader.nama} onChange={e => setFormKader({...formKader, nama: e.target.value})} required style={{ padding: '10px', border: '1px solid #ccc', borderRadius: '4px' }} />
                        <input type="text" placeholder="Password (Misal: 20042004)" value={formKader.password} onChange={e => setFormKader({...formKader, password: e.target.value})} required style={{ padding: '10px', border: '1px solid #ccc', borderRadius: '4px' }} />
                        
                        <select required value={formKader.pendampingId} onChange={e => setFormKader({...formKader, pendampingId: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc', borderRadius: '4px' }}>
                          <option value="" disabled>-- Pilih Pendamping Awal (MAPABA) --</option>
                          {dataPendamping.filter(p => !p.jenjangTugas || p.jenjangTugas === 'MAPABA').map(p => <option key={p.id} value={p.username}>{p.nama}</option>)}
                        </select>
                        
                        <button disabled={isSubmitting} type="submit" style={{ backgroundColor: isSubmitting ? '#95a5a6' : '#2ecc71', color: 'white', border: 'none', padding: '12px', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}>{isSubmitting ? 'Memproses...' : 'Buat Akun'}</button>
                      </form>
                    </div>

                    <div style={{ backgroundColor: '#f0fbf4', padding: '20px', border: '1px solid #c8e6c9', borderRadius: '8px' }}>
                      <h4 style={{ marginTop: 0, color: '#1e824c', borderBottom: '1px dashed #a5d6a7', paddingBottom: '8px' }}>📗 Import Massal (Excel)</h4>
                      <p style={{fontSize: '0.8rem', color: '#555'}}>Format Kolom di Excel wajib persis seperti ini (perhatikan besar/kecil huruf): <br/><b>NIM | Nama | TanggalLahir | Pendamping</b></p>
                      <form onSubmit={handleImportExcel} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        <input type="file" accept=".xlsx, .xls" required style={{ padding: '8px', border: '1px dashed #1e824c', borderRadius: '4px', backgroundColor: '#fff' }} />
                        <button disabled={isSubmitting} type="submit" style={{ backgroundColor: isSubmitting ? '#95a5a6' : '#1e824c', color: 'white', padding: '10px', borderRadius: '4px', fontWeight: 'bold', cursor: isSubmitting ? 'not-allowed' : 'pointer' }}>
                          🚀 Proses Data Excel
                        </button>
                        {importProgress && <div style={{fontSize: '0.8rem', color: '#e67e22', fontWeight: 'bold', textAlign: 'center'}}>{importProgress}</div>}
                      </form>
                    </div>
                    ...
                    <div style={{ backgroundColor: '#fff5f5', padding: '20px', border: '1px solid #ffcdd2', borderRadius: '8px' }}>
                      ...
                      <button onClick={handleBersihkanDataKaderLama} disabled={isSubmitting} style={{ backgroundColor: '#c62828', color: 'white', padding: '10px', borderRadius: '4px', fontWeight: 'bold', cursor: isSubmitting ? 'not-allowed' : 'pointer', width: '100%', border: 'none' }}>
                  ...
                  <div style={{ flex: '2 1 600px', backgroundColor: '#fff', border: '1px solid #eee', borderRadius: '8px', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ padding: '15px', borderBottom: '1px solid #eee', backgroundColor: '#fdfdfd', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
                      <div>
                        <h4 style={{ margin: 0, color: '#1e824c' }}>🔄 Daftar Kader, Plotting & Jenjang</h4>
                        <p style={{fontSize: '0.75rem', color: '#666', margin: '5px 0 0 0'}}>Filter & kelola pendamping atau naikkan jenjang kader.</p>
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                        <select 
                          ...
                          style={{ padding: '8px 12px', border: '1px solid #ccc', borderRadius: '20px', outline: 'none', fontSize: '0.85rem', cursor: 'pointer' }}
                        >
                        ...
                        <input 
                          ...
                          style={{ padding: '8px 12px', border: '1px solid #ccc', borderRadius: '20px', outline: 'none', fontSize: '0.85rem' }} 
                        />
                      </div>
                    </div>
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem', minWidth: '850px' }}>
                        <thead>
                          <tr style={{ backgroundColor: '#f8f9fa', color: '#333', textAlign: 'left' }}>
                            <th style={{ padding: '12px 10px' }}>NIM / Tahun</th>
                            <th style={{ padding: '12px 10px' }}>Nama Kader</th>
                            <th style={{ padding: '12px 10px' }}>Jenjang (Ubah)</th>
                            <th style={{ padding: '12px 10px' }}>Pendamping Saat Ini</th>
                            <th style={{ padding: '12px 10px', textAlign: 'center' }}>Status</th>
                            <th style={{ padding: '12px 10px', textAlign: 'center' }}>Hapus</th>
                          </tr>
                        </thead>
                        <tbody>
                            ...
                            filteredKader.map((k) => {
                              ...
                              return (
                                <tr key={k.id} style={{ borderBottom: '1px solid #eee' }}>
                                  <td style={{ padding: '12px 10px', fontWeight: 'bold', color: '#555' }}>
                                    {k.nim} <br/> <span style={{fontSize: '0.7rem', color: '#1e824c'}}>Agt. {thnMasuk}</span>
                                  </td>
                                  <td style={{ padding: '12px 10px', fontWeight: 'bold', color: '#333' }}>{k.nama}</td>
                                  
                                  <td style={{ padding: '12px 10px' }}>
                                    <select 
                                      value={k.jenjang || "MAPABA"} 
                                      onChange={(e) => handleUbahJenjangKader(k.nim, e.target.value)}
                                      style={{ padding: '6px', border: '1px solid #3498db', borderRadius: '4px', backgroundColor: '#eaf4fc', fontWeight: 'bold', cursor: 'pointer', width: '100%', maxWidth: '120px', fontSize: '0.8rem', color: '#2c3e50' }}
                                    >
                                      <option value="MAPABA">MAPABA</option>
                                      <option value="PKD">PKD</option>
                                      <option value="SIG">SIG</option>
                                  ...
                                  <td style={{ padding: '12px 10px' }}>
                                     <select 
                                       ...
                                       style={{ padding: '6px', border: '1px solid #2ecc71', borderRadius: '4px', backgroundColor: '#fff', fontWeight: 'bold', cursor: 'pointer', width: '100%', maxWidth: '150px', fontSize: '0.8rem' }}
                                     >
                                       <option value="">- Kosong -</option>
                                       {dataPendamping
                                        .filter(p => (p.jenjangTugas || 'MAPABA') === (k.jenjang || 'MAPABA'))
                                        .map(p => <option key={p.id} value={p.username}>{p.nama}</option>)
                                       }
                                     </select>
                                  </td>
                                  
                                  <td style={{ padding: '12px 10px', textAlign: 'center' }}>
                                    <button onClick={() => handleUbahStatusAkun(k.id, k.status)} style={{ padding: '4px 8px', border: 'none', borderRadius: '12px', fontSize: '0.75rem', fontWeight: 'bold', cursor: 'pointer', backgroundColor: k.status === 'Aktif' ? '#e8f5e9' : '#ffebee', color: k.status === 'Aktif' ? '#2e7d32' : '#c62828' }}>
              ...
              {tabAkun === 'pendamping' && (
                <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
                  ...
                  <div style={{ flex: '1 1 300px', backgroundColor: '#fdfdfd', padding: '20px', border: '1px solid #eee', borderRadius: '8px', alignSelf: 'flex-start' }}>
                    ...
                    <form onSubmit={handleBuatAkunPendamping} style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '10px' }}>
                      <input type="text" placeholder="Nama Lengkap Pendamping" value={formPendamping.nama} onChange={e => setFormPendamping({...formPendamping, nama: e.target.value})} required style={{ padding: '10px', border: '1px solid #ccc', borderRadius: '4px' }} />
                      <input type="text" placeholder="Username (contoh: ridwan.aw)" value={formPendamping.username} onChange={e => setFormPendamping({...formPendamping, username: e.target.value})} required style={{ padding: '10px', border: '1px solid #ccc', borderRadius: '4px' }} />
                      <input type="text" placeholder="Password Sementara" value={formPendamping.password} onChange={e => setFormPendamping({...formPendamping, password: e.target.value})} required style={{ padding: '10px', border: '1px solid #ccc', borderRadius: '4px' }} />
                      
                      <select required value={formPendamping.jenjangTugas} onChange={e => setFormPendamping({...formPendamping, jenjangTugas: e.target.value})} style={{ padding: '10px', border: '1px solid #ccc', borderRadius: '4px', fontWeight: 'bold', color: '#2c3e50' }}>
                      ...
                      <button disabled={isSubmitting} type="submit" style={{ backgroundColor: isSubmitting ? '#95a5a6' : '#1e824c', color: 'white', border: 'none', padding: '12px', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}>{isSubmitting ? 'Memproses...' : 'Buat Akun'}</button>
                    </form>
                  </div>
                  ...
                  <div style={{ flex: '2 1 500px', overflowX: 'auto', backgroundColor: '#fff', border: '1px solid #eee', borderRadius: '8px' }}>
                    <div style={{ padding: '15px', borderBottom: '1px solid #eee', backgroundColor: '#fdfdfd', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <h4 style={{ margin: 0, color: '#1e824c' }}>📋 Daftar Pendamping</h4>
                      <input 
                        type="text" 
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem', minWidth: '600px' }}>
                      <thead>
                        <tr style={{ backgroundColor: '#f8f9fa', color: '#333', textAlign: 'left' }}>
                          <th style={{ padding: '15px' }}>Nama Pendamping</th>
                          <th style={{ padding: '15px' }}>Username</th>
                          <th style={{ padding: '15px' }}>Tugas Jenjang</th>
                        ...
                        {filteredPendamping.length === 0 ? (
                          ...
                          filteredPendamping.map((p) => (
                            <tr key={p.id} style={{ borderBottom: '1px solid #eee' }}>
                              <td style={{ padding: '15px', fontWeight: 'bold' }}>{p.nama}</td>
                              ...
                              <td style={{ padding: '15px', textAlign: 'center' }}>
                                <button onClick={() => handleUbahStatusAkun(p.id, p.status || 'Aktif')} style={{ padding: '4px 8px', border: 'none', borderRadius: '12px', fontSize: '0.75rem', fontWeight: 'bold', cursor: 'pointer', backgroundColor: (!p.status || p.status === 'Aktif') ? '#e8f5e9' : '#ffebee', color: (!p.status || p.status === 'Aktif') ? '#2e7d32' : '#c62828' }}>
          ...
          {activeMenu === 'kurikulum' && (
            <div style={{ backgroundColor: '#fff', borderRadius: '8px', border: '1px solid #ddd', minHeight: '500px', overflow: 'hidden' }}>
              <div style={{ backgroundColor: '#4a637d', padding: '15px 20px', color: 'white', fontWeight: 'bold', fontSize: '0.95rem' }}>MANAJEMEN KURIKULUM RAYON</div>
              <div style={{ padding: '20px' }}>
                <p style={{fontSize: '0.85rem', color: '#666', marginBottom: '20px'}}>Pilih materi dari referensi pusat, atau tambahkan materi muatan lokal khusus rayon Anda.</p>
                
                <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap' }}>
                  <button onClick={() => setTabKurikulum('MAPABA')} style={{ padding: '10px 15px', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer', backgroundColor: tabKurikulum === 'MAPABA' ? '#1e824c' : '#f4f6f9', color: tabKurikulum === 'MAPABA' ? 'white' : '#555', flex: '1 1 auto' }}>📘 MAPABA</button>
                  <button onClick={() => setTabKurikulum('PKD')} style={{ padding: '10px 15px', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer', backgroundColor: tabKurikulum === 'PKD' ? '#1e824c' : '#f4f6f9', color: tabKurikulum === 'PKD' ? 'white' : '#555', flex: '1 1 auto' }}>📙 PKD</button>
                  <button onClick={() => setTabKurikulum('SIG')} style={{ padding: '10px 15px', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer', backgroundColor: tabKurikulum === 'SIG' ? '#1e824c' : '#f4f6f9', color: tabKurikulum === 'SIG' ? 'white' : '#555', flex: '1 1 auto' }}>📕 SIG</button>
                  <button onClick={() => setTabKurikulum('SKP')} style={{ padding: '10px 15px', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer', backgroundColor: tabKurikulum === 'SKP' ? '#1e824c' : '#f4f6f9', color: tabKurikulum === 'SKP' ? 'white' : '#555', flex: '1 1 auto' }}>👩 SKP</button>
                  <button onClick={() => setTabKurikulum('NON-FORMAL')} style={{ padding: '10px 15px', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer', backgroundColor: tabKurikulum === 'NON-FORMAL' ? '#1e824c' : '#f4f6f9', color: tabKurikulum === 'NON-FORMAL' ? 'white' : '#555', flex: '1 1 auto' }}>📗 Non-Formal</button>
                </div>
                
                <div style={{ overflowX: 'auto', border: '1px solid #eee', borderRadius: '8px', marginBottom: '30px' }}>
                  <div style={{backgroundColor: '#eef2f3', padding: '15px', borderBottom: '1px solid #ddd'}}>
                    <h4 style={{margin: 0, color: '#0d1b2a'}}>✅ Kurikulum Rayon Saat Ini ({tabKurikulum})</h4>
                  </div>
                  <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem', minWidth: '450px' }}>
                    <thead>
                      <tr style={{ backgroundColor: '#f8f9fa', color: '#555' }}>
                        <th style={{ padding: '12px', borderBottom: '2px solid #ddd', width: '40px' }}>No</th><th style={{ padding: '12px', borderBottom: '2px solid #ddd' }}>Kode</th><th style={{ padding: '12px', borderBottom: '2px solid #ddd' }}>Nama Kegiatan & Muatan</th><th style={{ padding: '12px', borderBottom: '2px solid #ddd', textAlign: 'center' }}>Bobot</th><th style={{ padding: '12px', borderBottom: '2px solid #ddd', textAlign: 'center' }}>Aksi</th>
                      </tr>
                    </thead>
                    <tbody>
                      {!listKurikulum[tabKurikulum] || listKurikulum[tabKurikulum].length === 0 ? (
                        ...
                        listKurikulum[tabKurikulum].map((materi, idx) => (
                          <tr key={materi.id} style={{ borderBottom: '1px solid #eee' }}>
                            ...
                            <td style={{ padding: '12px', fontWeight: 'bold', color: '#004a87' }}>{materi.kode}</td>
                            ...
                            <td style={{ padding: '12px', textAlign: 'center', fontWeight: 'bold' }}>{materi.bobot}</td>
                            <td style={{ padding: '12px', textAlign: 'center' }}><button onClick={() => handleHapusMateri(materi.id)} style={{ color: '#e74c3c', border: 'none', background: 'none', cursor: 'pointer', fontWeight: 'bold' }}>Hapus</button></td>
                          </tr>
                        ))
                      )}
                    </tbody>
                ...
                <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
                  <div style={{ flex: '2 1 400px', backgroundColor: '#fdfdfd', padding: '20px', borderRadius: '8px', border: '1px dashed #b2c2cf' }}>
                    <h4 style={{ color: '#0d1b2a', marginTop: 0, marginBottom: '15px' }}>📌 Referensi Kurikulum Standar Pusat</h4>
                    <div style={{ overflowX: 'auto', backgroundColor: '#fff', borderRadius: '4px', border: '1px solid #eee' }}>
                      <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem' }}>
                        <thead><tr style={{ backgroundColor: '#0d1b2a', color: 'white' }}><th style={{ padding: '10px' }}>Kode</th><th style={{ padding: '10px' }}>Materi Pusat</th><th style={{ padding: '10px', textAlign: 'center' }}>Aksi</th></tr></thead>
                        <tbody>
                            ...
                            masterKurikulumPusat.filter(m => m.jenjang === tabKurikulum).map((materiPusat) => {
                              ...
                              return (
                                <tr key={materiPusat.id} style={{ borderBottom: '1px solid #eee', backgroundColor: isAlreadyAdded ? '#f9f9f9' : 'white' }}>
                                  <td style={{ padding: '10px', fontWeight: 'bold', color: isAlreadyAdded ? '#999' : '#333' }}>{materiPusat.kode}</td>
                                  <td style={{ padding: '10px' }}>
                                    <div style={{fontWeight: 'bold', color: isAlreadyAdded ? '#999' : '#333'}}>{materiPusat.nama}</div>
                                    <div style={{fontSize: '0.75rem', color: '#888', whiteSpace: 'pre-wrap'}}>{materiPusat.muatan}</div>
                                  </td>
                                  <td style={{ padding: '10px', textAlign: 'center' }}>
                                    {isAlreadyAdded ? (
                                      <span style={{ color: '#27ae60', fontWeight: 'bold', fontSize: '0.8rem' }}>✅ Dipakai</span>
                                    ) : (
                                      <button 
                                        onClick={() => handleTarikMateriPusat(materiPusat)} 
                                        disabled={isSavingKurikulum}
                                        style={{ backgroundColor: '#3498db', color: 'white', border: 'none', padding: '5px 10px', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.75rem' }}
                                      >
                                        ➕ Pakai
                                      </button>
                                    )}
                  ...
                  <div style={{ flex: '1 1 250px', backgroundColor: '#f0fbf4', padding: '20px', border: '1px solid #c8e6c9', borderRadius: '8px', alignSelf: 'flex-start' }}>
                    <h4 style={{ marginTop: 0, color: '#1e824c', borderBottom: '1px dashed #a5d6a7', paddingBottom: '8px' }}>📝 Tambah Materi Lokal/Lainnya</h4>
                    <form onSubmit={handleTambahMateriLokal} style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '15px' }}>
                      <input type="text" placeholder="Kode (Misal: LOKAL-01)" required value={formMateri.kode} onChange={(e) => setFormMateri({...formMateri, kode: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc', borderRadius: '4px' }} />
                      <input type="text" placeholder="Nama Materi Lokal" required value={formMateri.nama} onChange={(e) => setFormMateri({...formMateri, nama: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc', borderRadius: '4px' }} />
                      <textarea rows={2} placeholder="Muatan / Pembahasan (Opsional)" value={formMateri.muatan} onChange={(e) => setFormMateri({...formMateri, muatan: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc', borderRadius: '4px', resize: 'vertical' }} />
                      <input type="number" placeholder="Bobot SKS/Jam" required value={formMateri.bobot} onChange={(e) => setFormMateri({...formMateri, bobot: Number(e.target.value)})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc', borderRadius: '4px' }} />
                      <button disabled={isSavingKurikulum} type="submit" style={{ backgroundColor: '#1e824c', color: 'white', border: 'none', padding: '10px', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}>Tambahkan Lokal</button>
                    </form>
                  </div>
            ...
            <div style={{ background: 'white', padding: '20px', borderRadius: '8px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
              ...
              <div className="no-print" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #eee', paddingBottom: '15px', marginBottom: '20px', flexWrap: 'wrap', gap: '15px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                  ...
                  <select value={selectedKaderNilai} onChange={(e) => setSelectedKaderNilai(e.target.value)} style={{ padding: '8px 12px', border: '1px solid #ccc', borderRadius: '4px', fontWeight: 'bold', minWidth: '200px', outline: 'none', cursor: 'pointer' }}>
              ...
              {tabRaportAdmin === 'raport' && (
                <div id="area-cetak-raport" style={{ overflowX: 'auto', border: '1px solid #eee', borderRadius: '4px', padding: '20px' }}>
                  ...
                  <div className="print-kop-surat" style={{ textAlign: 'center', marginBottom: '30px' }}>
                    ...
                    <table className="tabel-biodata" style={{ textAlign: 'left', margin: '30px auto 20px 0', width: '100%', maxWidth: '600px', fontSize: '0.95rem' }}>
                      <tbody>
                  ...
                  <table className="tabel-cetak" style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem', minWidth: '600px' }}>
                    <thead>
                    ...
                    <tbody>
                      ...
                      <tr style={{ borderTop: '2px solid #ccc' }}>
                        ...
                        <td className="col-cetak" style={{ padding: '10px 15px' }}></td>
                      ...
                      <tr style={{ borderTop: '1px solid #ccc', borderBottom: '1px solid #ccc' }}>
                        <td colSpan={5} className="col-cetak" style={{ padding: '15px', textAlign: 'center', fontWeight: 'bold', color: '#333', fontSize: '0.95rem' }}>IPK (Indeks Prestasi Kader)</td>
              ...
              {tabRaportAdmin === 'persentase' && (
                <div style={{ backgroundColor: '#fafafa', border: '1px solid #ddd', borderRadius: '4px', padding: '20px' }}>
                  <h4 style={{ margin: '0 0 20px 0', color: '#333', borderBottom: '1px solid #eee', paddingBottom: '10px' }}>Laporan Persentase Keaktifan dari Pendamping</h4>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', maxWidth: '600px' }}>
                    ...
                    ) : (<p style={{ color: '#888', fontStyle: 'italic', fontSize: '0.9rem' }}>Pendamping belum menginput tabel penilaian.</p>)}
                    <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#fff', borderLeft: '4px solid #007bff', fontSize: '0.85rem', color: '#555' }}>
                ...
                <div style={{ backgroundColor: '#fafafa', border: '1px solid #ddd', borderRadius: '4px', padding: '20px' }}>
                  ...
                  <form onSubmit={handleSimpanPengaturanCetak} style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '600px' }}>
                    <div style={{ backgroundColor: '#fff3cd', padding: '15px', borderRadius: '4px', borderLeft: '4px solid #f1c40f', fontSize: '0.85rem', color: '#856404' }}>
                      <b>PENTING (Format Gambar yang Benar):</b><br/>
                      Sistem website tidak bisa membaca file Word (.docx) atau PDF untuk Kop Surat. Anda wajib menyimpan file Word tersebut menjadi sebuah <b>Gambar (PNG/JPG)</b>. <br/>
                      <i>Cara: Di Word, blok area Kop/Tanda Tangan -&gt; Klik Kanan -&gt; "Save as Picture" -&gt; Simpan sebagai PNG.</i>
                    </div>

                    <div>
                      <label style={{ fontWeight: 'bold', display: 'block', marginBottom: '5px', color: '#333', fontSize: '0.9rem' }}>Kop Surat (Gambar Header Atas)</label>
                      <p style={{ fontSize: '0.75rem', color: '#777', margin: '0 0 10px 0' }}>Upload gambar PNG format lanskap (memanjang) resolusi tinggi.</p>
                      ...
                      <input type="file" accept="image/png, image/jpeg" onChange={(e) => setFileKop(e.target.files ? e.target.files[0] : null)} style={{ padding: '10px', border: '1px dashed #ccc', width: '100%', backgroundColor: '#fff' }} />
                    </div>
                    
                    <div>
                      <label style={{ fontWeight: 'bold', display: 'block', marginBottom: '5px', color: '#333', fontSize: '0.9rem' }}>Gambar Footer / Tanda Tangan (Bawah)</label>
                      <p style={{ fontSize: '0.75rem', color: '#777', margin: '0 0 10px 0' }}>Upload gambar PNG yang memanjang berisi teks Kota, Tanggal, Tanda Tangan, dan Stempel.</p>
                      ...
                      <input type="file" accept="image/png, image/jpeg" onChange={(e) => setFileFooter(e.target.files ? e.target.files[0] : null)} style={{ padding: '10px', border: '1px dashed #ccc', width: '100%', backgroundColor: '#fff' }} />
                    </div>
          ...
          {activeMenu === 'master-tugas' && (
            <div style={{ background: 'white', padding: '30px', borderRadius: '8px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
              <h3 style={{ color: '#1e824c', margin: 0, borderBottom: '2px solid #eee', paddingBottom: '15px', marginBottom: '20px' }}>Penugasan Kader Rayon</h3>
              <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
                <div style={{ flex: '1 1 300px', backgroundColor: '#fdfdfd', padding: '20px', border: '1px solid #eee', borderRadius: '8px' }}>
                  <h4 style={{marginTop: 0, marginBottom: '15px', color: '#333'}}>Tambah Tugas</h4>
                  <form onSubmit={handleTambahTugas} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    <input type="text" placeholder="Nama Tugas (Misal: Resume NDP)" required value={formTugas.nama_tugas} onChange={(e) => setFormTugas({...formTugas, nama_tugas: e.target.value})} style={{ padding: '12px', border: '1px solid #ccc', borderRadius: '4px' }} />
                    <input type="text" placeholder="Batas Waktu (Misal: 10 April)" required value={formTugas.deadline} onChange={(e) => setFormTugas({...formTugas, deadline: e.target.value})} style={{ padding: '12px', border: '1px solid #ccc', borderRadius: '4px' }} />
                    <button type="submit" style={{ backgroundColor: '#2ecc71', color: 'white', padding: '12px', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}>Buat Tugas Baru</button>
                  </form>
                </div>
                <div style={{ flex: '2 1 100%', overflowX: 'auto', border: '1px solid #eee', borderRadius: '8px' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem', minWidth: '400px' }}>
                    ...
                    <tbody>
                      {listMasterTugas.length === 0 ? (<tr><td colSpan={3} style={{textAlign: 'center', padding: '20px'}}>Belum ada tugas.</td></tr>) : listMasterTugas.map((tugas) => (
                        <tr key={tugas.id} style={{ borderBottom: '1px solid #eee' }}>
                          <td style={{ padding: '15px', fontWeight: 'bold' }}>{tugas.nama_tugas}</td><td style={{ padding: '15px', color: '#e74c3c' }}>{tugas.deadline}</td>
                          <td style={{ padding: '15px', textAlign: 'center' }}><button onClick={() => handleHapusTugas(tugas.id)} style={{ color: 'white', backgroundColor: '#e74c3c', padding: '5px 10px', borderRadius: '4px', border: 'none', cursor: 'pointer', fontWeight: 'bold' }}>Hapus</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {/* ========================================================= */}
          {/* MENU 6: KELOLA PERPUSTAKAAN */}
          {/* ========================================================= */}
          {activeMenu === 'perpus' && (
            <div style={{ background: 'white', padding: '30px', borderRadius: '8px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
              <h3 style={{ color: '#1e824c', margin: 0, borderBottom: '2px solid #eee', paddingBottom: '15px', marginBottom: '20px' }}>Kelola Perpustakaan & Arsip</h3>
              <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
                <div style={{ flex: '1 1 300px', backgroundColor: '#fdfdfd', padding: '20px', border: '1px solid #eee', borderRadius: '8px' }}>
                  <h4 style={{marginTop: 0, marginBottom: '15px', color: '#333'}}>Upload Materi Baru</h4>
                  <form onSubmit={handleTambahPerpus} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    <input type="text" placeholder="Nama Folder (Misal: Materi MAPABA)" required value={formPerpus.folder} onChange={(e) => setFormPerpus({...formPerpus, folder: e.target.value})} style={{ padding: '12px', border: '1px solid #ccc', borderRadius: '4px' }} />
                    <input type="text" placeholder="Judul Buku/Materi" required value={formPerpus.nama_file} onChange={(e) => setFormPerpus({...formPerpus, nama_file: e.target.value})} style={{ padding: '12px', border: '1px solid #ccc', borderRadius: '4px' }} />
                    <input type="file" required onChange={(e) => setFilePerpus(e.target.files ? e.target.files[0] : null)} style={{ padding: '10px', border: '1px dashed #ccc', borderRadius: '4px' }} />
                    <button disabled={isUploadingPerpus} type="submit" style={{ backgroundColor: isUploadingPerpus ? '#95a5a6' : '#004a87', color: 'white', padding: '12px', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: isUploadingPerpus ? 'not-allowed' : 'pointer' }}>{isUploadingPerpus ? 'Mengupload ke Server...' : 'Upload ke Perpus'}</button>
                  </form>
                </div>
                <div style={{ flex: '2 1 100%', overflowX: 'auto', border: '1px solid #eee', borderRadius: '8px' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem', minWidth: '450px' }}>
                    <thead><tr style={{ backgroundColor: '#f8f9fa' }}><th style={{ padding: '15px', borderBottom: '2px solid #ddd' }}>Folder</th><th style={{ padding: '15px', borderBottom: '2px solid #ddd' }}>Materi / Buku</th><th style={{ padding: '15px', borderBottom: '2px solid #ddd', textAlign: 'center' }}>Aksi</th></tr></thead>
                    <tbody>
                      {listPerpus.length === 0 ? (<tr><td colSpan={3} style={{textAlign: 'center', padding: '20px'}}>Perpustakaan masih kosong.</td></tr>) : listPerpus.map((item) => (
                        <tr key={item.id} style={{ borderBottom: '1px solid #eee' }}>
                          <td style={{ padding: '15px', fontWeight: 'bold', color: '#1e824c' }}>📁 {item.folder}</td><td style={{ padding: '15px' }}>{item.nama_file}</td>
                          <td style={{ padding: '15px', textAlign: 'center' }}>
                            <a href={item.link_file} target="_blank" style={{ marginRight: '10px', backgroundColor: '#f1c40f', color: '#333', padding: '6px 10px', borderRadius: '4px', textDecoration: 'none', fontWeight: 'bold' }}>Cek</a>
                            <button onClick={() => handleHapusPerpus(item.id)} style={{ color: 'white', backgroundColor: '#e74c3c', padding: '6px 10px', borderRadius: '4px', border: 'none', cursor: 'pointer', fontWeight: 'bold' }}>Hapus</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          {activeMenu === 'saran' && (
            <div style={{ background: 'white', padding: '30px', borderRadius: '8px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
              <h3 style={{ color: '#1e824c', margin: 0, borderBottom: '2px solid #eee', paddingBottom: '15px', marginBottom: '20px' }}>Kotak Saran & Aspirasi Kader</h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
                {saranMasuk.map((saran) => (
                  <div key={saran.id} style={{ border: '1px solid #ddd', padding: '20px', borderRadius: '8px', backgroundColor: '#fafafa', borderLeft: '4px solid #f1c40f' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px', fontSize: '0.8rem', color: '#777', borderBottom: '1px dashed #ccc', paddingBottom: '10px' }}>
                      <span style={{ fontWeight: 'bold', color: '#004a87' }}>{saran.nama} <br/><span style={{fontSize: '0.7rem', color: '#999'}}>NIM: {saran.nim}</span></span>
                      <span>{saran.tanggal}</span>
                    </div>
                    <p style={{ margin: 0, fontSize: '0.95rem', color: '#333', fontStyle: 'italic', lineHeight: '1.5' }}>"{saran.saran}"</p>
                  </div>
                ))}
                {saranMasuk.length === 0 && <p style={{ color: '#999', textAlign: 'center', width: '100%', padding: '20px' }}>Belum ada saran yang masuk dari kader.</p>}
              </div>
            </div>
          )}
