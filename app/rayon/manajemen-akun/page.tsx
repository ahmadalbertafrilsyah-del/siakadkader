...
export default function PageManajemenAkunRayon() {
  ...
  return (
    <>
      ...
      {/* DESKTOP VIEW */}
      <div className="desktop-view" style={{ background: 'white', padding: '25px', borderRadius: '8px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '25px', flexWrap: 'wrap' }}>
          <button onClick={() => setTabAkun('kader')} style={{ padding: '8px 18px', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer', backgroundColor: tabAkun === 'kader' ? '#0000af' : '#f8f9fa', color: tabAkun === 'kader' ? '#f1c40f' : '#555', fontSize: '0.85rem', transition: '0.2s', border: 'none' }}>🎓 Kader Rayon</button>
          <button onClick={() => setTabAkun('pendamping')} style={{ padding: '8px 18px', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer', backgroundColor: tabAkun === 'pendamping' ? '#0000af' : '#f8f9fa', color: tabAkun === 'pendamping' ? '#f1c40f' : '#555', fontSize: '0.85rem', transition: '0.2s', border: 'none' }}>👩 Pendamping Rayon</button>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ backgroundColor: '#fff', padding: '25px', border: '1px solid #eaeaea', borderRadius: '10px' }}>
              <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
                <button type="button" onClick={() => setModeInputKader('baru')} style={{ flex: 1, padding: '10px 5px', fontSize: '0.75rem', fontWeight: 'bold', border: modeInputKader === 'baru' ? 'none' : '1px solid #ccc', borderRadius: '6px', cursor: 'pointer', backgroundColor: modeInputKader === 'baru' ? '#0000af' : '#fff', color: modeInputKader === 'baru' ? '#f1c40f' : '#555', transition: '0.2s' }}>Buat Manual</button>
                  ...
                  <div>
                    ...
                    {formKader.id_rayon === 'Luar Komisariat' && (
                      <div style={{ marginTop: '10px' }}>
                        <label style={{ fontSize: '0.75rem', color: '#e67e22', fontWeight: 'bold' }}>Ketik Nama Asal Rayon Luar</label>
                        <input type="text" placeholder="Misal: PMII Brawijaya" value={formKader.nama_rayon_luar} onChange={e => setFormKader({...formKader, nama_rayon_luar: e.target.value})} required style={{ width: '100%', padding: '8px', border: '1px dashed #e67e22', borderRadius: '6px', boxSizing: 'border-box', fontSize: '0.8rem', outline: 'none', marginTop: '5px' }} />
                      </div>
                    )}
                  </div>
            ...
            <div style={{ width: '100%', overflowX: 'auto', border: '1px solid #eaeaea', borderRadius: '8px', overflow: 'hidden' }}>
              <table className="tabel-utama" style={{ minWidth: '900px', width: '100%' }}>
                <thead>
                  <tr>
                    <th style={{ textAlign: 'center', width: '5%' }}>No</th>
                    <th style={{ textAlign: 'center', width: '15%' }}>NIM</th>
                    <th style={{ width: '30%', textAlign: 'left' }}>Nama Lengkap</th>
                      ...
                      <tr key={kader.nim}>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <div style={{ display: 'flex', gap: '5px', justifyContent: 'center' }}>
                            <button onClick={() => setEditKaderModal({ oldNim: kader.nim, id: kader.id, nim: kader.nim, nama: kader.nama, nia: kader.nia || '', angkatan: kader.angkatan || '', id_rayon: kader.id_rayon || adminRayonId, jenjang: kader.jenjang || 'MAPABA', riwayat_kaderisasi: kader.riwayat_kaderisasi || { MAPABA: true, PKD: false, SIG: false, SKP: false }, pendamping_mapaba_id: kader.pendamping_mapaba_id || [], pendamping_pkd_id: kader.pendamping_pkd_id || [], pendamping_sig_id: kader.pendamping_sig_id || [], pendamping_skp_id: kader.pendamping_skp_id || [] })} style={{ backgroundColor: '#f1c40f', color: '#0d1b2a', border: 'none', padding: '6px 12px', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.75rem' }}>✏️ Edit</button>
        ...
        {tabAkun === 'pendamping' && (
           ...
           <div style={{ width: '100%', overflowX: 'auto', border: '1px solid #eaeaea', borderRadius: '8px', overflow: 'hidden' }}>
             <table className="tabel-utama" style={{ minWidth: '400px', width: '100%' }}>
               <thead>
                 <tr>
      ...
      {/* MOBILE VIEW (CARD DIGANTI MENJADI TABEL SCROLL HORIZONTAL) */}
      <div className="mobile-view mobile-padded">
        
        {/* TABS HORIZONTAL SCROLL */}
        <div className="hide-scroll" style={{ display: 'flex', gap: '10px', overflowX: 'auto', whiteSpace: 'nowrap', paddingBottom: '10px' }}>
          <button onClick={() => setTabAkun('kader')} style={{ padding: '10px 20px', borderRadius: '20px', fontWeight: 'bold', cursor: 'pointer', backgroundColor: tabAkun === 'kader' ? '#0000af' : '#fff', color: tabAkun === 'kader' ? '#f1c40f' : '#555', fontSize: '0.85rem', transition: '0.2s', border: 'none', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}>🎓 Kader Rayon</button>
          <button onClick={() => setTabAkun('pendamping')} style={{ padding: '10px 20px', borderRadius: '20px', fontWeight: 'bold', cursor: 'pointer', backgroundColor: tabAkun === 'pendamping' ? '#0000af' : '#fff', color: tabAkun === 'pendamping' ? '#f1c40f' : '#555', fontSize: '0.85rem', transition: '0.2s', border: 'none', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}>👩 Pendamping Rayon</button>
        </div>
        ...
        {tabAkun === 'kader' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ backgroundColor: '#fff', padding: '20px', border: '1px solid #eaeaea', borderRadius: '12px', boxShadow: '0 4px 10px rgba(0,0,0,0.02)' }}>
              <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
                <button type="button" onClick={() => setModeInputKader('baru')} style={{ flex: 1, padding: '10px 5px', fontSize: '0.8rem', fontWeight: 'bold', border: modeInputKader === 'baru' ? 'none' : '1px solid #ccc', borderRadius: '8px', cursor: 'pointer', backgroundColor: modeInputKader === 'baru' ? '#0000af' : '#fff', color: modeInputKader === 'baru' ? '#f1c40f' : '#555', transition: '0.2s' }}>Manual</button>
                <button type="button" onClick={() => setModeInputKader('import')} style={{ flex: 1, padding: '10px 5px', fontSize: '0.8rem', fontWeight: 'bold', border: modeInputKader === 'import' ? 'none' : '1px solid #ccc', borderRadius: '8px', cursor: 'pointer', backgroundColor: modeInputKader === 'import' ? '#2ecc71' : '#fff', color: modeInputKader === 'import' ? '#fff' : '#555', transition: '0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>📗 Import Excel</button>
              </div>

              {modeInputKader === 'baru' ? (
                <form onSubmit={handleBuatAkunKader} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                  <input type="text" placeholder="NIM Kader" value={formKader.nim} onChange={e => setFormKader({...formKader, nim: e.target.value})} required style={{ width: '100%', padding: '12px', border: '1px solid #ccc', borderRadius: '8px', outline: 'none', fontSize: '0.85rem' }} />
                  <input type="text" placeholder="NIA (Opsional)" value={formKader.nia} onChange={e => setFormKader({...formKader, nia: e.target.value})} style={{ width: '100%', padding: '12px', border: '1px solid #ccc', borderRadius: '8px', outline: 'none', fontSize: '0.85rem' }} />
                  <input type="text" placeholder="Nama Lengkap" value={formKader.nama} onChange={e => setFormKader({...formKader, nama: e.target.value})} required style={{ width: '100%', padding: '12px', border: '1px solid #ccc', borderRadius: '8px', outline: 'none', fontSize: '0.85rem' }} />
                  <select value={formKader.id_rayon} onChange={e => setFormKader({...formKader, id_rayon: e.target.value})} required style={{ width: '100%', padding: '12px', border: '1px solid #ccc', borderRadius: '8px', fontSize: '0.85rem', outline: 'none', backgroundColor: '#fcfcfc' }}>
                     <option value="" disabled>-- Pilih Asal Rayon --</option>
                     {dataRayon.map(r => <option key={r.id_rayon} value={r.id_rayon}>{r.nama}</option>)}
                     <option value="Luar Komisariat">Delegasi Luar Komisariat</option>
                  </select>
                  {formKader.id_rayon === 'Luar Komisariat' && (
                    <input type="text" placeholder="Nama Rayon Luar" value={formKader.nama_rayon_luar} onChange={e => setFormKader({...formKader, nama_rayon_luar: e.target.value})} required style={{ width: '100%', padding: '12px', border: '1px dashed #e67e22', borderRadius: '8px', fontSize: '0.85rem', outline: 'none' }} />
                  )}
                  <input type="number" placeholder="Angkatan (Cth: 2026)" value={formKader.angkatan} onChange={e => setFormKader({...formKader, angkatan: e.target.value})} required style={{ width: '100%', padding: '12px', border: '1px solid #ccc', borderRadius: '8px', outline: 'none', fontSize: '0.85rem' }} />
                  
                  <div style={{ backgroundColor: '#fafafa', border: '1px solid #eee', borderRadius: '8px', padding: '15px' }}>
                    <label style={{ fontSize: '0.8rem', color: '#555', fontWeight: 'bold', display: 'block', marginBottom: '10px' }}>Pendamping MAPABA</label>
                    <div className="hide-scroll" style={{ maxHeight: '150px', overflowY: 'auto' }}>
                      {dataPendamping.map(p => (
                        <label key={p.id} style={{ display: 'flex', alignItems: 'center', fontSize: '0.85rem', marginBottom: '12px', cursor: 'pointer', color: '#333' }}>
                          <input type="checkbox" value={p.username} checked={formKader.pendamping_mapaba_id.includes(p.username)} onChange={(e) => { const val = e.target.value; if(e.target.checked) setFormKader(prev => ({...prev, pendamping_mapaba_id: [...prev.pendamping_mapaba_id, val]})); else setFormKader(prev => ({...prev, pendamping_mapaba_id: prev.pendamping_mapaba_id.filter(id => id !== val)})); }} style={{ marginRight: '10px', transform: 'scale(1.2)' }} />
                          {p.nama}
                        </label>
                      ))}
                    </div>
                  ...
                  <input type="text" placeholder="Password: Tgl Lahir (DDMMYYYY)" value={formKader.password} onChange={e => setFormKader({...formKader, password: e.target.value})} required style={{ width: '100%', padding: '12px', border: '1px solid #ccc', borderRadius: '8px', outline: 'none', fontSize: '0.85rem' }} />
                  <button disabled={isSubmitting} type="submit" style={{ backgroundColor: isSubmitting ? '#95a5a6' : '#2ecc71', color: 'white', border: 'none', padding: '15px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.9rem' }}>{isSubmitting ? 'Memproses...' : 'Daftarkan Kader'}</button>
                </form>
              ) : (
                <form onSubmit={handleImportExcel} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                  <div style={{ fontSize: '0.75rem', color: '#555', backgroundColor: '#fff3e0', padding: '12px', borderRadius: '8px', borderLeft: '4px solid #f39c12', lineHeight: '1.5' }}>
                    <b>Format Excel: NIM | NIA | Nama | Asal Rayon | Jenjang | Angkatan | TanggalLahir | Pendamping</b>
                  </div>
                  <input type="file" accept=".xlsx, .xls" required style={{ padding: '12px', border: '2px dashed #2ecc71', borderRadius: '8px', backgroundColor: '#fcfcfc', fontSize: '0.85rem', outline: 'none' }} />
                  <button type="button" onClick={handleDownloadTemplate} style={{ padding: '12px', backgroundColor: '#0000af', color: 'white', borderRadius: '8px', border: 'none', fontWeight: 'bold', fontSize: '0.85rem' }}>📥 Unduh Template</button>
                  <button disabled={isSubmitting} type="submit" style={{ backgroundColor: isSubmitting ? '#95a5a6' : '#2ecc71', color: 'white', padding: '15px', borderRadius: '8px', fontWeight: 'bold', fontSize: '0.9rem' }}>🚀 Mulai Import Data</button>
                  {importProgress && <div style={{textAlign: 'center', fontSize: '0.8rem', color: '#e67e22', fontWeight: 'bold'}}>{importProgress}</div>}
                </form>
              )}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <input type="text" placeholder="🔍 Cari NIM / Nama..." value={searchKader} onChange={(e) => setSearchKader(e.target.value)} style={{ padding: '12px', border: '1px solid #ccc', borderRadius: '8px', fontSize: '0.85rem', outline: 'none' }} />
              <div style={{ display: 'flex', gap: '10px' }}>
                <select value={filterJenjangKader} onChange={(e) => setFilterJenjangKader(e.target.value)} style={{ flex: 1, padding: '12px', border: '1px solid #ccc', borderRadius: '8px', fontSize: '0.85rem', outline: 'none', backgroundColor: '#fff' }}>
                ...
                <select value={itemsPerPage} onChange={(e) => { setItemsPerPage(Number(e.target.value)); setKaderPage(1); }} style={{ width: '100px', padding: '12px', border: '1px solid #0000af', color: '#0000af', borderRadius: '8px', outline: 'none', backgroundColor: '#f0f5ff', fontWeight: 'bold' }}>
                  <option value={10}>10 Baris</option><option value={50}>50 Baris</option><option value={100}>100 Baris</option>
                </select>
              </div>
            </div>

            {/* TABEL DATA KADER MOBILE (DIKEMBALIKAN KE TABEL, HAPUS MODE CARD) */}
            <div style={{ width: '100%', overflowX: 'auto', backgroundColor: '#fff', border: '1px solid #eaeaea', borderRadius: '12px', boxShadow: '0 4px 10px rgba(0,0,0,0.02)' }}>
                  ...
                  {currentKaderDisplay.length === 0 ? (
                    ...
                    currentKaderDisplay.map((kader, idx) => (
                      <tr key={kader.nim}>
                        ...
                        <td style={{ textAlign: 'center', fontWeight: 'bold' }}>{kader.nim}</td>
                        <td>
                          <div style={{fontWeight: 'bold', color: '#0d1b2a'}}>{kader.nama}</div>
                          <div style={{fontSize: '0.75rem', color: '#888'}}>NIA: {kader.nia || '-'} | Angkatan: {kader.angkatan}</div>
                        </td>
                        <td style={{ textAlign: 'center' }}>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <div style={{ display: 'flex', gap: '5px', justifyContent: 'center' }}>
                            <button onClick={() => setEditKaderModal({ oldNim: kader.nim, id: kader.id, nim: kader.nim, nama: kader.nama, nia: kader.nia || '', angkatan: kader.angkatan || '', id_rayon: kader.id_rayon || adminRayonId, jenjang: kader.jenjang || 'MAPABA', riwayat_kaderisasi: kader.riwayat_kaderisasi || { MAPABA: true, PKD: false, SIG: false, SKP: false }, pendamping_mapaba_id: kader.pendamping_mapaba_id || [], pendamping_pkd_id: kader.pendamping_pkd_id || [], pendamping_sig_id: kader.pendamping_sig_id || [], pendamping_skp_id: kader.pendamping_skp_id || [] })} style={{ backgroundColor: '#f1c40f', color: '#0d1b2a', border: 'none', padding: '6px 12px', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.75rem' }}>Edit</button>
                            <button onClick={() => handleHapusKaderTotal(kader)} style={{ backgroundColor: '#e74c3c', color: 'white', border: 'none', padding: '6px 12px', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.75rem' }}>Hapus</button>
                          </div>
                        </td>
                      </tr>
                    ))
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#fff', padding: '10px 15px', borderRadius: '8px', border: '1px solid #eaeaea' }}>
              <button disabled={kaderPage === 1} onClick={() => setKaderPage(kaderPage - 1)} style={{ padding: '8px 15px', borderRadius: '6px', border: '1px solid #ccc', background: '#f4f6f9', fontWeight: 'bold', color: '#555', fontSize: '0.8rem' }}>Seb</button>
              <span style={{ fontSize: '0.8rem', fontWeight: 'bold', color: '#777' }}>Hal {kaderPage}</span>
              <button onClick={() => setKaderPage(kaderPage + 1)} style={{ padding: '8px 15px', borderRadius: '6px', border: '1px solid #ccc', background: '#f4f6f9', fontWeight: 'bold', color: '#555', fontSize: '0.8rem' }}>Sel</button>
            </div>
          </div>
        )}

        {/* TAB PENDAMPING RAYON (DIKEMBALIKAN KE TABEL) */}
        {tabAkun === 'pendamping' && (
           <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
             <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '12px', border: '1px solid #eaeaea', boxShadow: '0 4px 10px rgba(0,0,0,0.02)' }}>
               <h4 style={{ margin: '0 0 15px 0', color: '#1e824c', fontSize: '1rem', borderBottom: '1px solid #eee', paddingBottom: '10px' }}>➕ Akun Pendamping</h4>
               <form onSubmit={handleBuatAkunPendamping} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                 <input type="text" placeholder="Nama Lengkap" value={formPendamping.nama} onChange={e => setFormPendamping({...formPendamping, nama: e.target.value})} required style={{ width: '100%', padding: '12px', border: '1px solid #ccc', borderRadius: '8px', fontSize: '0.85rem', outline: 'none' }} />
                 <input type="text" placeholder="Username Login" value={formPendamping.username} onChange={e => setFormPendamping({...formPendamping, username: e.target.value})} required style={{ width: '100%', padding: '12px', border: '1px solid #ccc', borderRadius: '8px', fontSize: '0.85rem', outline: 'none' }} />
                 <select required value={formPendamping.jenjangTugas} onChange={e => setFormPendamping({...formPendamping, jenjangTugas: e.target.value})} style={{ width: '100%', padding: '12px', border: '1px solid #ccc', borderRadius: '8px', fontSize: '0.85rem', outline: 'none', backgroundColor: '#fff' }}><option value="MAPABA">MAPABA</option><option value="PKD">PKD</option><option value="SIG">SIG</option></select>
                 <input type="text" placeholder="Password Login" value={formPendamping.password} onChange={e => setFormPendamping({...formPendamping, password: e.target.value})} required style={{ width: '100%', padding: '12px', border: '1px solid #ccc', borderRadius: '8px', fontSize: '0.85rem', outline: 'none' }} />
                 <button disabled={isSubmitting} type="submit" style={{ backgroundColor: '#0000af', color: 'white', border: 'none', padding: '15px', borderRadius: '8px', fontWeight: 'bold', fontSize: '0.9rem', marginTop: '5px' }}>{isSubmitting ? 'Memproses...' : 'Daftarkan Pendamping'}</button>
               </form>
             </div>

             <div style={{ width: '100%', overflowX: 'auto', backgroundColor: '#fff', border: '1px solid #eaeaea', borderRadius: '12px', boxShadow: '0 4px 10px rgba(0,0,0,0.02)' }}>
               <table className="tabel-utama" style={{ minWidth: '500px', width: '100%' }}>
                 <thead>
                   <tr>
                     <th style={{ textAlign: 'center' }}>Nama Pendamping</th>
                     <th style={{ textAlign: 'center' }}>Username</th>
                 ...
                 <tbody>
                   {dataPendamping.length === 0 ? (
                     <tr><td colSpan={5} style={{textAlign: 'center', padding: '30px', color: '#999'}}>Belum ada pendamping.</td></tr>
                   ) : (
                     dataPendamping.map(p => (
                       <tr key={p.id}>
                         <td style={{ fontWeight: 'bold', color: '#0d1b2a', textAlign: 'center' }}>{p.nama}</td>
      ...
      {/* MODAL EDIT KADER (GLOBAL UNTUK HP DAN DESKTOP) */}
      {editKaderModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999, padding: '15px' }}>
          <div className="hide-scroll" style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '12px', width: '100%', maxWidth: '650px', maxHeight: '90vh', overflowY: 'auto', position: 'relative', boxShadow: '0 20px 40px rgba(0,0,0,0.2)' }}>
            <button onClick={() => setEditKaderModal(null)} style={{ position: 'absolute', top: '15px', right: '15px', background: '#f8f9fa', border: 'none', borderRadius: '50%', width: '30px', height: '30px', cursor: 'pointer', fontSize: '1rem', fontWeight: 'bold', color: '#555' }}>✖</button>
            <h3 style={{ marginTop: 0, color: '#0000af', borderBottom: '2px solid #eaeaea', paddingBottom: '10px', marginBottom: '15px', fontSize: '1.1rem' }}>⚙️ Edit Data Kader</h3>
            <form onSubmit={handleSimpanEditKader} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                 <div><label style={{ fontSize: '0.8rem', fontWeight: 'bold', color: '#555', display: 'block', marginBottom: '5px' }}>Nama Lengkap</label><input type="text" required value={editKaderModal.nama} onChange={e => setEditKaderModal({...editKaderModal, nama: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc', borderRadius: '8px', outline: 'none', fontSize: '0.85rem' }} /></div>
                 <div><label style={{ fontSize: '0.8rem', fontWeight: 'bold', color: '#555', display: 'block', marginBottom: '5px' }}>NIM</label><input type="text" required value={editKaderModal.nim} onChange={e => setEditKaderModal({...editKaderModal, nim: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc', borderRadius: '8px', outline: 'none', fontSize: '0.85rem' }} /></div>
                 <div><label style={{ fontSize: '0.8rem', fontWeight: 'bold', color: '#555', display: 'block', marginBottom: '5px' }}>NIA</label><input type="text" value={editKaderModal.nia} onChange={e => setEditKaderModal({...editKaderModal, nia: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc', borderRadius: '8px', outline: 'none', fontSize: '0.85rem' }} /></div>
                 <div>
                   <label style={{ fontSize: '0.8rem', fontWeight: 'bold', color: '#555', display: 'block', marginBottom: '5px' }}>Asal Rayon</label>
                   <select value={editKaderModal.id_rayon} onChange={e => setEditKaderModal({...editKaderModal, id_rayon: e.target.value})} required style={{ width: '100%', padding: '10px', border: '1px solid #ccc', borderRadius: '8px', outline: 'none', fontSize: '0.85rem', backgroundColor: '#fff' }}>
                     {dataRayon.map(r => <option key={r.id_rayon} value={r.id_rayon}>{r.nama}</option>)}
                     <option value="Luar Komisariat">Delegasi Luar Komisariat</option>
                   </select>
                 </div>
                 <div><label style={{ fontSize: '0.8rem', fontWeight: 'bold', color: '#555', display: 'block', marginBottom: '5px' }}>Tahun Angkatan</label><input type="number" required value={editKaderModal.angkatan} onChange={e => setEditKaderModal({...editKaderModal, angkatan: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc', borderRadius: '8px', outline: 'none', fontSize: '0.85rem' }} /></div>
                 <div><label style={{ fontSize: '0.8rem', fontWeight: 'bold', color: '#555', display: 'block', marginBottom: '5px' }}>Password / Tgl Lahir</label><input type="text" value={editKaderModal.tanggalLahir || ''} onChange={e => setEditKaderModal({...editKaderModal, tanggalLahir: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ccc', borderRadius: '8px', outline: 'none', fontSize: '0.85rem' }} /></div>

              <div style={{ backgroundColor: '#f0fbf4', border: '1px solid #27ae60', padding: '15px', borderRadius: '12px' }}>
                <h4 style={{ margin: '0 0 10px 0', color: '#1e824c', fontSize: '0.9rem' }}>🎓 Histori Lulus Jenjang</h4>
                <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
                  {['MAPABA', 'PKD', 'SIG', 'SKP'].map(jenjang => (
                    <label key={jenjang} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem', color: '#333' }}>
                ...
                <div style={{ marginTop: '15px' }}>
                  <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#1e824c' }}>Status Jenjang TERTINGGI Saat Ini:</label>
                  <select value={editKaderModal.jenjang} onChange={e => setEditKaderModal({...editKaderModal, jenjang: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #27ae60', borderRadius: '8px', fontWeight: 'bold', color: '#27ae60', marginTop: '5px', outline: 'none', backgroundColor: '#fff', fontSize: '0.85rem' }}>
                    <option value="MAPABA">MAPABA</option><option value="PKD">PKD</option><option value="SIG">SIG</option><option value="SKP">SKP</option>
                  </select>
                </div>
              </div>

              <div style={{ backgroundColor: '#fcfcfc', border: '1px solid #eaeaea', padding: '15px', borderRadius: '12px' }}>
                <h4 style={{ margin: '0 0 10px 0', color: '#0d1b2a', fontSize: '0.9rem' }}>👨‍🏫 Plotting Pendamping</h4>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {['mapaba', 'pkd', 'sig', 'skp'].map(jenjangKey => {
                      const fieldName = `pendamping_${jenjangKey}_id`;
                      return (
                        <div key={jenjangKey} style={{ border: '1px solid #ddd', borderRadius: '8px', padding: '10px', backgroundColor: '#fff' }}>
                          <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#0000af', display: 'block', marginBottom: '8px' }}>Pendamping {jenjangKey.toUpperCase()}</label>
                          <div className="hide-scroll" style={{ maxHeight: '100px', overflowY: 'auto' }}>
                            {dataPendamping.map(p => (
                              <label key={p.id} style={{ display: 'flex', alignItems: 'flex-start', fontSize: '0.8rem', marginBottom: '8px', cursor: 'pointer', color: '#555', fontWeight: 'bold' }}>
                                <input type="checkbox" value={p.username} checked={(editKaderModal[fieldName] || []).includes(p.username)} onChange={(e) => { const val = e.target.value; const currentArr = editKaderModal[fieldName] || []; if(e.target.checked) setEditKaderModal({...editKaderModal, [fieldName]: [...currentArr, val]}); else setEditKaderModal({...editKaderModal, [fieldName]: currentArr.filter((id: string) => id !== val)}); }} style={{ marginRight: '8px', accentColor: '#0000af' }} />
                                {p.nama}
                              </label>
                            ))}
                          </div>
              ...
              <button disabled={isSubmitting} type="submit" style={{ backgroundColor: isSubmitting ? '#95a5a6' : '#0000af', color: 'white', border: 'none', padding: '12px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.95rem', marginTop: '10px' }}>
