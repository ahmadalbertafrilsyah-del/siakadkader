            ...
            <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
              <div style={{ flex: '1 1 300px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div style={{ backgroundColor: '#fdfdfd', padding: '20px', border: '1px solid #eaeaea', borderRadius: '10px' }}>
                  <h4 style={{ margin: '0 0 15px 0', color: '#1e824c', fontSize: '0.9rem' }}>📥 Tarik Tes Standar Komisariat</h4>
                      ...
                      masterTesPusat.map((tesPusat: any) => {
                        ...
                        return (
                          <div key={tesPusat.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', paddingBottom: '10px', borderBottom: '1px dashed #eee' }}>
                            <div><strong style={{ fontSize: '0.85rem' }}>{tesPusat.judul}</strong><br/><span style={{ fontSize: '0.75rem', color: '#555' }}>Jenjang: {tesPusat.jenjang}</span></div>
                            {isDitarik ? (
                              <span style={{ fontSize: '0.7rem', color: '#27ae60', fontWeight: 'bold', backgroundColor: '#e8f5e9', padding: '4px 8px', borderRadius: '4px' }}>Ditarik</span>
                            ) : (
                              <button onClick={() => handleTarikTesPusat(tesPusat)} style={{ backgroundColor: '#f1c40f', color: '#333', border: 'none', padding: '4px 8px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 'bold', cursor: 'pointer' }}>Tarik</button>
                            )}
                          </div>
                        )
                      })
                ...
                <div style={{ backgroundColor: '#fdfdfd', padding: '20px', border: '1px solid #eaeaea', borderRadius: '10px' }}>
                  <h4 style={{ marginTop: 0, color: '#0d1b2a', fontSize: '0.9rem', marginBottom: '15px' }}>➕ Buat Tes / Soal Lokal</h4>
                  <form onSubmit={handleBuatTes} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <input type="text" placeholder="Judul Tes (Misal: Post Test Mapaba)" required value={formTes.judul} onChange={e => setFormTes({...formTes, judul: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ddd', borderRadius: '4px', fontSize: '0.85rem' }} />
                    <select required value={formTes.jenjang} onChange={e => setFormTes({...formTes, jenjang: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ddd', borderRadius: '4px', fontSize: '0.85rem', cursor: 'pointer' }}>
                    ...
                    <textarea rows={4} placeholder="Daftar Pertanyaan (Enter untuk memisah soal)..." required value={formTes.soal} onChange={e => setFormTes({...formTes, soal: e.target.value})} style={{ width: '100%', padding: '10px', border: '1px solid #ddd', borderRadius: '4px', resize: 'vertical', fontSize: '0.85rem' }} />
                    <button type="submit" style={{ backgroundColor: '#0000af', color: 'white', border: 'none', padding: '10px', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.85rem' }}>Simpan Tes Lokal</button>
                  </form>
                </div>
              </div>

              <div style={{ flex: '2 1 450px', overflowX: 'auto', border: '1px solid #eaeaea', borderRadius: '10px', padding: '10px', alignSelf: 'flex-start' }}>
                <table className="tabel-utama" style={{ minWidth: '550px' }}>
                  <thead><tr><th style={{ textAlign: 'center' }}>Judul Tes & Jenjang</th><th style={{ textAlign: 'center', width: '10%' }}>Soal</th><th style={{ textAlign: 'center', width: '20%' }}>Status</th><th style={{ textAlign: 'center', width: '25%' }}>Aksi</th></tr></thead>
                  <tbody>
                    {listTes.length === 0 ? (
                      ...
                      listTes.map((tes) => (
                        <tr key={tes.id}>
                          <td><div style={{ fontWeight: 'bold', color: '#0d1b2a', fontSize: '0.9rem' }}>{tes.judul}</div><div style={{ fontSize: '0.75rem', color: '#888' }}>{tes.jenjang}</div></td>
                          <td style={{ textAlign: 'center', fontWeight: 'bold', color: '#e67e22' }}>{tes.daftar_soal?.length || 0}</td>
                          ...
                          <td style={{ textAlign: 'center' }}>
                            <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                              <button onClick={() => handleLihatHasilTes(tes)} style={{ color: 'white', backgroundColor: '#3498db', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.75rem' }}>Hasil</button>
                              <button onClick={async () => { if (window.confirm("Hapus tes ini?")) { await deleteDoc(doc(db, "master_tes", tes.id)); catatLogAktivitas(`Menghapus Tes: ${tes.judul}`); } }} style={{ color: 'white', backgroundColor: '#e74c3c', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.75rem' }}>Hapus</button>
                            </div>
                          </td>
                        </tr>
                      ))
